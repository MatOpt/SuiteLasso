...
function [obj,x,xi,u,info,runhist] = Fused_Lasso_SSNAL_Wrapper(Ainput,b,n,lambda1,lambda2,options,x,xi,u)
...
if isQR == 0
   ...
   if ~exist('u','var'); u = zeros(n,1); end;
   if ~exist('xi','var'); xi = zeros(length(b),1); end;
   [obj,x,xi,u,info,runhist] = Fused_Lasso_SSNAL(Ainput,b,n,lambda1,lambda2,options,x,xi,u);
else
   ...
   [obj,x,xi,u,info,runhist] = Fused_Lasso_SSNAL(R,b,n,lambda1,lambda2,options);
end
